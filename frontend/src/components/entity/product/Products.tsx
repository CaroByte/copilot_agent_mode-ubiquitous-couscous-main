...
export default function Products() {
  ...
  const handleAddToCart = (productId: number) => {
    ...
    if (quantity > 0) {
      ...
      if (product) {
        const finalPrice = (product.discount && product.discount > 0) ? product.price * (1 - product.discount) : product.price;
        addToCart(productId, product.name, finalPrice, quantity);
        setQuantities(prev => ({
          ...prev,
          [productId]: 0
        }));
