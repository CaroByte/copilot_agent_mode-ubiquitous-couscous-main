...
describe('Headquarters API', () => {
    ...
    it('should return 404 for non-existing headquarters (PUT)', async () => {
        const response = await request(app).put('/headquarters/999').send({ name: 'x' });
        expect(response.status).toBe(404);
    });

    it('should return 404 for non-existing headquarters (DELETE)', async () => {
