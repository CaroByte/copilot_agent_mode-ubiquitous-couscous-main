...
describe('Order API', () => {
    ...
    it('should return 404 for non-existing order (GET)', async () => {
        const response = await request(app).get('/orders/999');
        expect(response.status).toBe(404);
    });

    it('should return 404 for non-existing order (PUT)', async () => {
        const response = await request(app).put('/orders/999').send({ name: 'x' });
        expect(response.status).toBe(404);
    });

    it('should return 404 for non-existing order (DELETE)', async () => {
